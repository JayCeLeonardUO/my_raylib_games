...
struct HotloadViewer {
    std::string glb_path = "./hotload.glb";
    Model model = {0};
    ...
    void try_load() {
        ...
        if (model.meshCount > 0) {
            ...
            reload_count++;
            TraceLog(LOG_INFO, "Loaded GLB: %s (meshes: %d, reload #%d)",
                     glb_path.c_str(), model.meshCount, reload_count);
        } else {
...
int main(int argc, char* argv[]) {
    ...
    while (!WindowShouldClose()) {
        ...
        const char* status = viewer.model_loaded ?
